...
static int atmel_copybit(struct copybit_context_t *dev, void const *list) 
{
    ...
    if(src_bpp != dst_bpp){
        ALOGE_IF(DEBUG_MDP_ERRORS,"In atmel_copybit, src format != dst format, we just return and let opengl do the convert");
        return -EINVAL;
    }
    const size_t bpp = src_bpp;
    for (int i=0 ; i<l->count ; i++) {
        ALOGD_IF(DEBUG_MDP_ERRORS,"%d: src={w=%d, h=%d, f=%d, membase=%p, rect={%d,%d,%d,%d}}\n"
        ...
        struct copybit_region_t const *region) 
{
    ...
    if (ctx) {
        ...
        if (ctx->mFlags != MDP_ROT_NOP){
            ALOGE_IF(DEBUG_MDP_ERRORS,"The flag is 0x%x",ctx->mFlags);
            return -EINVAL;
        }
