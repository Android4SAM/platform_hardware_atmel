...
int s_device_open(const hw_module_t*, const char*, hw_device_t**);
...
hw_module_methods_t s_module_methods = {
    open            : s_device_open
};

int s_device_open(const hw_module_t* module, const char* name,
        hw_device_t** device)
{
    alsa_device_t *dev;
...
static alsa_handle_t _defaultsOut = {
    ...
    /* Change by embest
         * For video A/V sync, one audio frame size is 4096
         * the buffers numbers set by android is 4
         * so change this to 1024 * 4 =4096
         */
