...
#define LOG_TAG "atmel_audio_policy_hal"
//#define LOG_NDEBUG 0
...
struct atmel_ap_module {
    struct audio_policy_module module;
};

struct atmel_ap_device {
    struct audio_policy_device device;
};

struct atmel_audio_policy {
...
static inline struct atmel_audio_policy * to_lap(struct audio_policy *pol)
{
    return reinterpret_cast<struct atmel_audio_policy *>(pol);
}

static inline const struct atmel_audio_policy * to_clap(const struct audio_policy *pol)
{
    return reinterpret_cast<const struct atmel_audio_policy *>(pol);
}
...
{
    struct atmel_audio_policy *lap = to_lap(pol);
...
static audio_policy_dev_state_t ap_get_device_connection_state(
                                            ...
                                            const char *device_address)
{
    const struct atmel_audio_policy *lap = to_clap(pol);
...
static int create_atmel_ap(const struct audio_policy_device *device,
                            struct audio_policy_service_ops *aps_ops,
                            void *service,
                            struct audio_policy **ap)
{
    struct atmel_audio_policy *lap;
    int ret;

    if (!service || !aps_ops)
        return -EINVAL;

    lap = (struct atmel_audio_policy *)calloc(1, sizeof(*lap));
    if (!lap)
        return -ENOMEM;
...
static int destroy_atmel_ap(const struct audio_policy_device *ap_dev,
                             struct audio_policy *ap)
{
    struct atmel_audio_policy *lap = to_lap(ap);
...
static int atmel_ap_dev_close(hw_device_t* device)
{
    if (device)
        free(device);
    return 0;
...
static int atmel_ap_dev_open(const hw_module_t* module, const char* name,
                                    hw_device_t** device)
{
    struct atmel_ap_device *dev;

    if (strcmp(name, AUDIO_POLICY_INTERFACE) != 0)
        return -EINVAL;

    dev = (struct atmel_ap_device *)calloc(1, sizeof(*dev));
    if (!dev)
        return -ENOMEM;
    ...
    dev->device.common.close = atmel_ap_dev_close;
    dev->device.create_audio_policy = create_atmel_ap;
    dev->device.destroy_audio_policy = destroy_atmel_ap;
...
static struct hw_module_methods_t atmel_ap_module_methods = {
        open: atmel_ap_dev_open
};

struct atmel_ap_module HAL_MODULE_INFO_SYM = {
    module: {
        common: {
            ...
            name: "ATMEL Audio Policy HAL",
            author: "The Android Open Source Project",
            methods: &atmel_ap_module_methods,
            dso : NULL,
            reserved : {0},
        },
    },
