...
//#define LOG_FUNCTION_NAME    LOGD("%s", __func__);
...
int v4l2_overlay_open(struct hwc_win_info_t_heo *win, const char *dir)
{
    ...
    if (v4l2_overlay_querycap(win->fd) < 0) {
        LOGD("%s:: we can not use %s", __func__, name);
        goto error;
    }
    ...
    LOGD("%s, open %s successful: fd:%d", __func__, name, win->fd);
...
int v4l2_overlay_close(struct hwc_win_info_t_heo *win)
{
    ...
    if (0 <= win->fd){
        ...
        if (v4l2_overlay_req_buf(win) < 0) {
            LOGE("%s:Failed requesting buffers\n", __func__);
        }
...
int v4l2_overlay_stream_on(struct hwc_win_info_t_heo *win)
{
    ...
    if (!win->steamEn) {
        ...
        if (ret) {
            LOGE("%s: Stream on Failed!/%d", __func__, ret);
        } else {
            win->steamEn = true;
        }
    } else {
...
int v4l2_overlay_stream_off(struct hwc_win_info_t_heo *win)
{
    ...
    if (win->steamEn) {
        ...
        if (ret) {
            LOGE("%s: Stream Off Failed!/%d", __func__, ret);
        } else {
            win->steamEn = false;
        }
    } else {
        LOGV("%s: stream has already off");
    }
