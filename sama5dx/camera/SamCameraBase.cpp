 ...
 */
 ...
 /*
  * Contains implementation of a class SamCameraBase that encapsulates
  * functionality common to all camera device versions ("ov camera series",
  * "webcam", "video file", "cam2.0" etc.).  Instances of this class (for each
  * camera) are created during the construction of the
  * SamCameraFactory instance.  This class serves as an entry point for all
  * camera API calls that are common across all versions of the
  * camera_device_t/camera_module_t structures.
  */
  
//#define LOG_NDEBUG 0
#define LOG_TAG "SamCameraBase"

#include "SamCameraBase.h"

namespace android {

SamCameraBase::SamCameraBase(int cameraId,
...
SamCameraBase::~SamCameraBase()
{
    ALOGV("%s", __FUNCTION__);
}

status_t SamCameraBase::getCameraInfo(struct camera_info* info)
{
