...
SamSensorBase::SamSensorBase(SamCameraProtocol1* camera_hal)
    : mObjectLock(),
      ...
      mBufSize(NULL),
      mBufVirAddr(NULL),
      mDeliveringFrames(false),
      mTakingPictureEnabled(false)
{
    ALOGV("%s", __FUNCTION__);
}

SamSensorBase::~SamSensorBase()
{
    ALOGV("~SamSensorBase destructor");
    for (uint32_t i = 0; i < mSupportedWhiteBalanceScale.size(); ++i) {
...
status_t SamSensorBase::Initialize()
{
    ALOGV("%s", __FUNCTION__);

    if (isInitialized()) {
        ALOGW("%s: camera sensor is already initialized: mState = %d",
             __FUNCTION__, mState);
        return NO_ERROR;
    }
...
status_t SamSensorBase::startDeliveringFrames(bool one_burst)
{
    ALOGV("%s", __FUNCTION__);
...
status_t SamSensorBase::stopDeliveringFrames()
{
    ALOGV("%s", __FUNCTION__);
...
void SamSensorBase::setExposureCompensation(const float ev) {
    ALOGV("%s", __FUNCTION__);
...
void SamSensorBase::initializeWhiteBalanceModes(const char* mode,
                                                       const float r_scale,
                                                       const float b_scale) {
...
void SamSensorBase::setWhiteBalanceMode(const char* mode) {
 ...
 */
void SamSensorBase::changeWhiteBalance(uint8_t& y,
                                              uint8_t& u,
                                              uint8_t& v) const {
...
status_t SamSensorBase::getCurrentPreviewFrame(void* buffer)
{
    buffer = NULL;
    return NO_ERROR;
}
...
status_t SamSensorBase::commonStartDevice(int width,
                                                 int height,
                                                 uint32_t pix_fmt)
{
    ...
    switch (pix_fmt) {
        case V4L2_PIX_FMT_YUYV:
            mFrameBufferSize = (width * height * 16) / 8;
            break;
        default:
...
status_t SamSensorBase::startWorkerThread(bool one_burst)
{
    ALOGV("%s", __FUNCTION__);
...
status_t SamSensorBase::stopWorkerThread()
{
    ALOGV("%s", __FUNCTION__);
...
bool SamSensorBase::inWorkerThread()
{
...
status_t SamSensorBase::WorkerThread::readyToRun()
{
    ALOGV("Starting Sam camera device worker thread...");
    ...
    if (pipe(thread_fds) == 0) {
        ...
        ALOGV("Sam camera device's worker thread has been started.");
        return NO_ERROR;
    } else {
...
status_t SamSensorBase::WorkerThread::stopThread()
{
    ALOGV("Stopping Sam camera device's worker thread...");
    ...
    if (mThreadControl >= 0) {
        ...
        if (wres == sizeof(msg)) {
            ...
            if (res == NO_ERROR) {
                ...
                ALOGV("Sam camera device's worker thread has been stopped.");
            } else {
...
SamSensorBase::WorkerThread::SelectRes
SamSensorBase::WorkerThread::Select(int fd, int timeout)
{
    fd_set fds[1];
