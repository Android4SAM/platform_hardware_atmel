...
class SamCameraProtocol1;

/* Encapsulates an abstract class SamSensorBase that defines
 * functionality expected from an real physical camera device:
 *  - Obtaining and setting camera device parameters
 *  - Capturing frames
 *  - Streaming video
 *  - etc.
...
class SamSensorBase {
public:
     /* Constructs SamSensorBase instance.
     * Param:
     *  camera_hal - camera protocol1 that implements the camera HAL API, and
     *      manages (contains) this object.
     */
    explicit SamSensorBase(SamCameraProtocol1* camera_hal);

    /* Destructs SamSensorBase instance. */
    virtual ~SamSensorBase();

    /***************************************************************************
     * Sensor Base abstract interface
     **************************************************************************/
...
public:
    ...
    /* Starts the sensor.
     * This method tells the sensor to start capturing frames of the given
     * dimensions for the given pixel format. Note that this method doesn't start
     * the delivery of the captured frames to the camera hal. Call
   ...
   /* Stops the sensor.
     ...
     * this method doesn't stop delivering frames to the camera hal. Always
    ...
    /***************************************************************************
     * Sensor Base public API
     **************************************************************************/

public:
    /* Initializes SamSensorBase instance.
    ...
    /* Starts delivering frames captured from the camera sensors.
    ...
    /* Stops delivering frames captured from the camera sensors.
    ...
    /* Sets the white balance mode for the sensor.
     */
    ...
    /* Gets pixel format of the frame that sensor streams to this class.
     * Throughout camera framework, there are three different forms of pixel
     * format representation:
     *  - Original format, as reported by the actual camera sensor. Values for
     ...
     * Since the camera sensor gets its data from the actual physical device, it gets
     ...
     *  method is valid only in case if sensor has been started.
     */
    ...
    inline bool isDelivering() const {
        return mDeliveringFrames;
    }

    /****************************************************************************
     * SamCameraSensor private API
     ***************************************************************************/
protected:
    ...
    inline uint8_t changeExposure(const uint8_t& inputY) const {
        int value = (static_cast<float>(inputY) * mExposureCompensation);
...
protected:
    ...
    /* Encapsulates a worker thread used by the  camera sensor.
     */
    ...
    class WorkerThread : public Thread {
        ...
        public:
            inline explicit WorkerThread(SamSensorBase* sensorbase)
                : Thread(true),   // Callbacks may involve Java calls.
                  mSensorBase(sensorbase),
                  mThreadControl(-1),
                  mControlFD(-1)
            {
            }
            ...
            {
                mOneBurst = one_burst;
                return run("WorkerThread", ANDROID_PRIORITY_URGENT_DISPLAY, 0);
            }
        ...
        private:
            ...
            bool threadLoop()
            {
                ...
                if (mSensorBase->inWorkerThread()) {
                    /* Respect "one burst" parameter (see startThread). */
                    if (mOneBurst && !mSensorBase->isDelivering())
                        return true;
...
protected:
    ...
    /* Object state. */
    SensorDeviceState   mState;
    uint32_t                    mBufNums;
    size_t*                     mBufSize;
    void**                      mBufVirAddr;
    bool                        mDeliveringFrames;
    bool                        mTakingPictureEnabled;
};
